# 对话 Agent 评测系统 · 运行与开发说明

本文件是 `agent/` 子项目的详细运行文档。仓库首页说明见 [`../README.md`](../README.md)，技术方案说明见 [`../TECHNICAL_REPORT.md`](../TECHNICAL_REPORT.md)。
...
侧边栏可控制：用户类型勾选 / 评测深度（1 或 3 次 Judge）/ 占位符取值组数 / 并行测试数（1/2/4/8/16）。
...
- **并行度建议按接口稳定性调整**。Streamlit 支持 case 级并行执行；并行度越高，总耗时越短，但也更容易遇到模型接口限流、网络抖动或结构化输出异常。正式评测建议先用 1-4 路并行验证，再根据接口稳定性提高并行度。
- **规则解析结果建议快速复核**。系统会对解析出的规则做质量校验，并对严重结构问题自动修复；对于较长或包含多个业务要求的规则，建议在评测前确认规则粒度是否符合预期。
