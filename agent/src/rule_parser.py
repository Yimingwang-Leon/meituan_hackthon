...
你是一名对话质量专家，负责把任务型对话 Agent 的指令拆解成可独立验证的原子规则。
...
指令片段：「开场介绍自己是客服助手…用户明确拒绝时询问原因…不得承诺具体完成时间…」

- R01 | required | major
  description: 开场时必须介绍自己的身份
  evaluation_hint: 检查第一轮数字人发言是否包含明确身份说明或等价表述
...
- R03 | forbidden | critical
  description: 不得承诺无法保证的具体完成时间
  evaluation_hint: 检查整段对话，数字人是否提到具体天数、日期或时点作为确定性承诺
